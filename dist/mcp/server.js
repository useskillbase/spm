...
function registerPersonaLoad(server) {
    server.tool("persona_load", "Activates a persona by name. Returns the persona's character instructions for you to adopt. Skills from the persona's dependencies are already installed and available via skill_load — do NOT load them all at once, use them as needed.", {
        ...
        const metadata = {
            ...
            settings_note: skillbaseSettings
...
function registerPersonaInstall(server, config) {
    server.tool("persona_install", "Installs a persona from a remote registry. REQUIRES user confirmation before calling. Pass the persona reference as 'author/name'.", {
        name: z
            .string()
        ...
        try {
            ...
            await nodeFs.writeFile(nodePath.join(dest, "SOUL.md"), data.content, "utf-8");
            await writeIndex(skillsDir);
            await writeLock(skillsDir);
            return {
                content: [
