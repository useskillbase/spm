...
export async function createServer(): Promise<McpServer> {
  ...
  const skillLines = index.skills.map((s) => {
    ...
    if (s.file_patterns) line += `|${s.file_patterns.join(",")}`;
    return line;
  });
  const skillIndex =
    skillLines.length > 0
...
function registerSkillList(server: McpServer): void {
  server.tool(
    "skill_list",
    "Returns a compact list of all installed skills with their trigger descriptions, tags, and token estimates. Use this to discover which skills are available before loading one.",
    {},
    async () => {
      const index = await getSkillIndex();
...
function registerSkillLoad(
  ...
  server.tool(
    "skill_load",
    "Loads a skill's full instructions into context. Pass the skill name (e.g. 'docx'). Use compact=true when context budget is tight. Returns content, permissions, works_with composition hints, and confidence score.",
    {
...
function registerSkillContext(
  ...
  server.tool(
    "skill_context",
    "Returns information about the current skill session: which skills are loaded, how many tokens are used, and available token budget.",
    {},
    async () => {
      const index = await getSkillIndex();
      const totalAvailable = index.skills.reduce(
        (sum, s) => sum + s.tokens_estimate,
        0,
      );
...
function registerSkillSearch(server: McpServer, config: SkillsConfig): void {
  server.tool(
    "skill_search",
    "Search for skills by query. Matches against skill names, tags, trigger descriptions, and file patterns. Use scope='remote' to search remote registries, scope='all' for both local and remote. Returns confidence score when available.",
    {
      query: z
        .string()
      ...
      scope: z
        ...
        .default("local")
        .describe(
          "Search scope: local (installed), remote (registries), or all",
        ),
    },
    async ({ query, scope }) => {
      const results: { local?: unknown[]; remote?: unknown[] } = {};
      ...
      if (scope === "local" || scope === "all") {
        ...
        const scored = index.skills.map((skill) => {
          ...
          if (skill.tags.some((t) => t === q)) score += 8;
          if (skill.tags.some((t) => t.includes(q))) score += 4;
          if (skill.trigger.toLowerCase().includes(q)) score += 3;
          if (skill.file_patterns?.some((p) => p.includes(q))) score += 6;
          return { skill, score };
        });
        ...
        results.local = await Promise.all(
          matches.map(async (s) => {
...
function registerSkillFeedback(server: McpServer): void {
  server.tool(
    "skill_feedback",
    "Records feedback after using a skill. Call this after completing a task with a loaded skill. Result: success (task completed), partial (partially helpful), failure (skill didn't help), false_trigger (wrong skill for the task).",
    {
      name: z.string().describe("Skill name, e.g. 'docx'"),
      result: z
...
function registerSkillInstall(server: McpServer, config: SkillsConfig): void {
  server.tool(
    "skill_install",
    "Installs a skill from a remote registry. REQUIRES user confirmation before calling. Suggest the skill to the user first, then call this only after they approve. Pass the skill reference as 'author/name' (e.g. 'community/code-reviewer').",
    {
      name: z
        .string()
    ...
    async ({ name, version }) => {
      ...
      try {
        ...
        await fs.writeFile(
          path.join(dest, (manifest as { entry: string }).entry),
          data.content,
          "utf-8",
        );
...
function registerPersonaList(server: McpServer): void {
  server.tool(
    "persona_list",
    "Lists all available personas. A persona defines the AI's character, tone, and associated skills.",
    {},
    async () => {
      const personas = await listPersonas();
...
function registerPersonaLoad(server: McpServer): void {
  server.tool(
    "persona_load",
