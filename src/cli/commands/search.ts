...
import type { RemoteSkillEntry, IndexSkillEntry } from "../../types/index.js";
...
export async function searchCommand(
  ...
  const choices = await multiselect({
    ...
    options: skills.map((s) => {
      const parts: string[] = [];
      if (s.installs != null) parts.push(`${s.installs} installs`);
  ...
  const selected = new Set(choices as string[]);
